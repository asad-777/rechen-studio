...
export const AnimatedThemeToggler = ({
  ...
  return (
    <button
      ...
      className={cn(
        "relative w-14 h-7 rounded-full bg-black/20 dark:bg-white/20 backdrop-blur-2xl border shadow-[inset_0_1px_4px_rgba(0,0,0,0.1)] flex items-center p-1 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-110 active:scale-95",
        className
      )}
      {...props}>
      <div 
        className={`flex items-center justify-center w-5 h-5 bg-white backdrop-blur-md rounded-full shadow-lg transform transition-transform duration-300 ${isDark ? 'translate-x-7' : 'translate-x-0'}`} 
      >
        {isDark ? (
          <Moon className="w-3 h-3 text-black" strokeWidth={2.5} />
        ) : (
          <Sun className="w-3 h-3 text-black" strokeWidth={2.5} />
        )}
      </div>
